...
import { transformInlineTextCell } from '../core/cell/transformer/inline_text';
...
function rowlenByRange(d, r1, r2, cfg) {
  const cfg_clone = $.extend(true, {}, cfg);
  ...
  for (let r = r1; r <= r2; r++) {
    ...
    for (let c = 0; c < d[r].length; c++) {
      ...
      if (cell != null && (cell.v != null || isInlineStringCell(cell))) {
        ...
        if (cell.mc) {
          if (c === cell.mc.c) {
            const st_cellWidth = colLocationByIndex(c)[0];
            const ed_cellWidth = colLocationByIndex(cell.mc.c + cell.mc.cs - 1)[1];
...
function computeRowlenByContent(d, r) {
  ...
  for (let c = 0; c < d[r].length; c++) {
    ...
    if (cell != null && (cell.v != null || isInlineStringCell(cell))) {
      const cellWidth = computeCellWidth(cell, c);
...
function computeColWidthByContent(d, c, rh) {
  let currentColLen = 0;
  const rowlenArr = computeRowlenArr(rh, c)
...
function getCellTextSplitArr(strValue, strArr, cellWidth, canvas) {
  for (let strI = 1; strI <= strValue.length; strI++) {
    const strV = strValue.substring(0, strI);
    const strtextMetrics = getMeasureText(strV, canvas).width;
...
function getMeasureText(value, ctx, fontset) {
  ...
  if (mtc != null) {
    ...
    const measureText = ctx.measureText(value); const cache = {};
    // var regu = "^[ ]+$";
    ...
    if (cache.actualBoundingBoxDescent == null || cache.actualBoundingBoxAscent == null || isNaN(cache.actualBoundingBoxDescent) || isNaN(cache.actualBoundingBoxAscent)) {
      ...
      const oneLineTextHeight = menuButton.getTextSize(commonWord, ctx.font)[1] * 0.8;
    ...
    if (ctx.textBaseline == 'alphabetic') {
      const descText = 'gjpqy'; const matchText = 'abcdABCD';
...
function getCellTextInfo(cell, ctx, option) {
  const cellWidth = option.cellWidth;
  const cellHeight = option.cellHeight;
  let isMode = ''; let isModeSplit = '';
  // console.log("initialinfo", cell, option);
  if (cellWidth == null) {
    isMode = 'onlyWidth';
    isModeSplit = '_';
  }
  const textInfo = Store.measureTextCellInfoCache[`${option.r}_${option.c}${isModeSplit}${isMode}`];
  if (textInfo != null) {
    return textInfo;
  }

  // let cell = Store.flowdata[r][c];
  let space_width = option.space_width; let space_height = option.space_height; // 宽高方向 间隙
  ...
  // 水平对齐
  const horizonAlign = checkstatusByCell(cell, 'ht');
  // 垂直对齐
  const verticalAlign = checkstatusByCell(cell, 'vt');

  const tb = checkstatusByCell(cell, 'tb');// wrap overflow
  const tr = checkstatusByCell(cell, 'tr');// rotate
  let rt = checkstatusByCell(cell, 'rt');// rotate angle

  let isRotateUp = 1; let isRotateDown = 0;
  ...
  let fontset; let cancelLine = '0'; let underLine = '0'; let fontSize = 11; let isInline = false; let value; const inlineStringArr = [];
  if (isInlineStringCell(cell)) {
    const sharedStrings = cell.ct.s; let similarIndex = 0;
    for (let i = 0; i < sharedStrings.length; i++) {
      const shareCell = sharedStrings[i];
      const scfontset = luckysheetfontformat(shareCell);
      const fc = shareCell.fc; const cl = shareCell.cl; const un = shareCell.un; let v = shareCell.v; const fs = shareCell.fs;
  ...
  if (tr == '3') { // vertical text
    ...
    let textH_all = 0;
    let colIndex = 0; let textH_all_cache = 0; const textH_all_Column = {}; const textH_all_ColumnHeight = [];
    if (isInline) {
      let preShareCell = null;
      for (let i = 0; i < inlineStringArr.length; i++) {
        const shareCell = inlineStringArr[i];
        let value = shareCell.v; let showValue = shareCell.v;
        ...
        const measureText = getMeasureText(value, ctx, shareCell.fontset);
      ...
      const measureText = getMeasureText(value, ctx);
      ...
      const oneWordWidth = getMeasureText(vArr[0], ctx).width;
    ...
    for (let i = 0; i < textH_all_ColumnHeight.length; i++) {
      ...
      const col = textH_all_Column[i]; let colMaxW = 0;
    ...
    if (tb == '2' || isInline) { // wrap
      ...
      let splitIndex = 0; const text_all_cache = 0; const text_all_split = {}; const text_all_splitLen = [];
      ...
      if (isInline) {
        ...
        while (i <= inlineStringArr.length) {
          ...
          if (shareCells[shareCells.length - 1].wrap === true) {
            ...
            if (shareCells.length == 1 || i == inlineStringArr.length) {
              ...
              const measureText = getMeasureText('M', ctx, sc.fontset);
          ...
          let textWidth = 0; let textHeight = 0;
          ...
          const width = textWidth * Math.cos(rt * Math.PI / 180) + textHeight * Math.sin(rt * Math.PI / 180);// consider text box wdith and line height

          const height = textWidth * Math.sin(rt * Math.PI / 180) + textHeight * Math.cos(rt * Math.PI / 180);// consider text box wdith and line height
        ...
        while (i <= value.length) {
          ...
          const measureText = getMeasureText(str, ctx);
      ...
      const split_all_size = []; let oneLinemaxWordCount = 0;
      ...
      for (let i = 0; i < splitLen; i++) {
        ...
        let sWidth = 0; let sHeight = 0; let maxDesc = 0; let maxAsc = 0; let lineHeight = 0; let maxWordCount = 0;
      ...
      // let cumColumnWidth = 0;
      let cumWordHeight = 0; let cumColumnWidth = 0;
      ...
      const rh = textW_all_inner * Math.sin(rtPI); let fixOneLineLeft = 0;
      ...
      if (rt != 0 && isRotateUp == '1') {
        ...
        for (let i = 0; i < splitLen; i++) {
          ...
          for (let c = splitLists.length - 1; c >= 0; c--) {
            ...
            let left, top;
            if (rt != 0) { // rotate
              let x; const y = cumWordHeight + size.asc;
      ...
      return transformNormalTextCell(ctx, cell, {
        cellWidth,
        cellHeight,
        spaceHeight: 2,
        spaceWidth: 2,
...
function drawLineInfo(wordGroup, cancelLine, underLine, option) {
  const left = option.left; const top = option.top; const width = option.width; const height = option.height; const asc = option.asc; const desc = option.desc; const fs = option.fs;
